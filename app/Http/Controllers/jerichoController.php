...
class jerichoController extends Controller
{
    ...
    public function index()
    {
        ...
        $chart = (new LarapexChart)->pieChart()
            ->setTitle('ذكر و انثى')
            ->setTitle('نسبة الذكر والانثى في محافظة اريحا')
            ->addData([
                \App\Models\family__data::where('place', '=', 'اريحا')->where('gender', '=', 'ذكر')->count(),
                \App\Models\family__data::where('place', '=', 'اريحا')->where('gender', '=', 'انثى')->count(),
            ])
            ->setLabels(['ذكر', 'انثى']);

        // statistics population city in the palestine
        $chart1 = (new LarapexChart)->pieChart()
            ->setTitle('عدد السكان في محافظة اريحا')
            ->addData([
        ...
        $chart2 = (new LarapexChart)->polarAreaChart()
            ->setTitle('نسبة الديانات في محافظة اريحا')
            ->addData([
                \App\Models\family__data::where('place', '=', 'اريحا')->where('religion', '=', 'مسلم')->count(),
                \App\Models\family__data::where('place', '=', 'اريحا')->where('religion', '=', 'مسيحي')->count(),
            ])
        ...
        $chart3 = (new LarapexChart)->pieChart()
            ->setTitle('نسبة الولادات طيله الحياه الزوجيه(للنساء14 فاكثر)ٍ في محافظة اريحا')
            ->addData([
        ...
        $chart4 = (new LarapexChart)->radialChart()
            ->setTitle('نسبة الولادات خلال ال 12 شهر السابقة (للنساء 14-54) في محافظة اريحا')
            ->addData([
        ...
        $chart5 = (new LarapexChart)->donutChart()
            ->setTitle('الحالة الزواجية في محافظة اريحا')
            ->addData([
        ...
        $chart6 = (new LarapexChart)->donutChart()
            ->setTitle('تكنولوجيا النعلومات لدى الاسرة(العدد) في محافظة اريحا')
            ->addData([
        ...
        $chart7 = (new LarapexChart)->pieChart()
            ->setTitle('نسبة عدد الوفايات في محافظة اريحا')
        ...
        return view('Palestine.jericho.jericho', $send);
    }
}
