...
class jeninController extends Controller
{
    ...
    public function index()
    {
        ...
        $chart = (new LarapexChart)->pieChart()
            ->setTitle('ذكر و انثى')
            ->setTitle('نسبة الذكر والانثى في محافظة جنين')
            ->addData([
        ...
        $chart1 = (new LarapexChart)->pieChart()
            ->setTitle('عدد السكان في محافظة جنين')
            ->addData([
        ...
        $chart2 = (new LarapexChart)->polarAreaChart()
            ->setTitle('نسبة الديانات في محافظة جنين')
            ->addData([
                \App\Models\family__data::where('place', '=', 'جنين')->where('religion', '=', 'مسلم')->count(),
                \App\Models\family__data::where('place', '=', 'جنين')->where('religion', '=', 'مسيحي')->count(),
            ])
        ...
        $chart3 = (new LarapexChart)->pieChart()
            ->setTitle('نسبة الولادات طيله الحياه الزوجيه(للنساء14 فاكثر)ٍ في محافظة جنين')
            ->addData([
        ...
        $chart4 = (new LarapexChart)->radialChart()
            ->setTitle('نسبة الولادات خلال ال 12 شهر السابقة (للنساء 14-54) في محافظة جنين')
            ->addData([
        ...
        $chart5 = (new LarapexChart)->donutChart()
            ->setTitle('الحالة الزواجية في محافظة جنين')
            ->addData([
        ...
        $chart6 = (new LarapexChart)->donutChart()
            ->setTitle('تكنولوجيا النعلومات لدى الاسرة(العدد) في محافظة جنين')
            ->addData([
        ...
        $chart7 = (new LarapexChart)->pieChart()
            ->setTitle('نسبة عدد الوفايات في محافظة جنين')
        ...
        return view('Palestine.jenin.jenin', $send);
    }
}
